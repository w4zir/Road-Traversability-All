...
class RoadQuality {
  public:
    ...
    const static double wbWidth =1.3;//1.86;// 1.3; //v1=1.3, v2=2, v3=2.5   for real road used 0,6
    const static double wbLength = 1.78;//3.25;//1.78; //v1=2.4, v2=3, v3=4    for real road used 0,9
    const static double vehWidth = 1.58;//2.3;//1.58; //v1=1.5, v2=2, v3=2.5   for real road used 0,8
    const static double vehLength = 3.3;//5.56;//3.3; //v1=3.5, v2=3, v3=4    for real road used 1,0
    const static double vehHeight = 1.9; //v1=3.5, v2=3, v3=4    for real road used 1,0
    const static double frontMaxAnlge = 30;
    const static double backMaxAnlge = 360;
    const static double wheelSurfaceRadius = 0.1;
    const static double wheelWidth =0.24;//0.3;// 0.24; //v1=0.2, v2=3, v3=4    for real road used 0.08
    const static double wheelRoadContactLength;
    const static double wheelAllowedHeight;
    const static double vehicleGroundClearance =0.25;//0.47;// 0.25;

    const static int wConfigCount = 25;
    const static int lConfigCount = 40;
    const static int tConfigCount = 7;
    ...
    const static double nSearchRadiusFactor = 1;
    const static double negObsTh = -0.2;
    const static double posObsTh = 0.25;
  public:
    ...
    void getPositiveObstacleIndex();
    void getStartConfigurations(string);
    void getGoalConfigurations(string);
    //RoadQuality();
    ...
    void projectCloudOnPlane() {
      ...
      //cout<<"coefficients at project time:\t"<<*coefficients<<endl;
    }
    void setTreeCloud() {
    ...
    double reorderPositiveObstacleIndex(int indx) {
//    std::vector<int> obsIdxReordered;
  RoadQuality::obsIdx.push_back(RoadQuality::obsIdx[indx]);
  RoadQuality::obsIdx.erase(RoadQuality::obsIdx.begin()+indx);
/*
  for (size_t i=indx;i<obsIdx.size();i++) {
    obsIdxReordered.push_back(obsIdx[i]);
  }
  for (size_t i=0;i<indx;i++) {
    obsIdxReordered.push_back(obsIdx[i]);
  }
  RoadQuality::obsIdx = obsIdxReordered; */
  cout << "no of obstacles after reorder\t"<<RoadQuality::obsIdx.size()<<endl;
    }
};
void RoadQuality::getStartConfigurations(string mode) {
  // save start configurations in startVector
    if (mode.compare("deterministic")) {
        for (size_t i=0;i<wConfigCount;i++) {
            for (size_t k=0;k<tConfigCount;k++) {
                startVector.push_back(i*tConfigCount+k);
            }
        }
    }
}

void RoadQuality::getGoalConfigurations(string mode) {
  // save Goal configurations in goalVector
    if (mode.compare("deterministic")) {
        for (size_t i=0;i<wConfigCount;i++) {
            for (size_t k=0;k<tConfigCount;k++) {
                goalVector.push_back((lConfigCount-1)*wConfigCount*tConfigCount + i*tConfigCount+k);
            }
        }
    }
}
...
void RoadQuality::getPlaneCoefficients(pcl::PointCloud<PointXYZ>::Ptr cloud) {
  ...
  cout << "coefficients after xy-transform are: "<<*coefficients<<endl;
}
...
void RoadQuality::getVehicleConfigurations()
{
  ...
  double tInc = thetaRange/(tConfigCount-1);
...
void RoadQuality::transformCloud2XYPlane() {
  ...
  pcl::transformPointCloud (*cloudXYZ, *xyTransformCloud, transform);
  cloudXYZ = xyTransformCloud;
  cout <<"points in transformed pointcloud:\t"<<cloudXYZ->points.size()<<endl;
}

void RoadQuality::getPositiveObstacleIndex() {
    /*
     * find indeces of all points that are part of obstacles
     */
            for (size_t i=0;i<cloudXYZ->points.size();i++) {
                double distFromPlane = ((coefficients->values[0]*cloudXYZ->points[i].x + coefficients->values[1]*cloudXYZ->points[i].y + coefficients->values[2]*cloudXYZ->points[i].z + coefficients->values[3])/(pCoeffMod));
                if (distFromPlane > vehicleGroundClearance && distFromPlane < vehHeight) {
                    obsIdx.push_back(i);
                }
              }
            cout<<obsIdx.size()<<"\tpoints are part of the positive obstacles"<<endl;
}
...
void RoadQuality::generatePRM() {
  ...
  int curPerc=0;
  for (size_t it=0;it<cloudIdx.size();++it) {
      curPerc = it*100/cloudIdx.size();
      if(curPerc!= prevPerc) {
          cout<<"PRM\t"<< curPerc <<"% complete"<<endl;
          prevPerc = curPerc;
      }
    //cout <<"selected point valid flag:\t"<<prmCloud->points[cloudIdx[it]].valid<<endl;
    if(prmCloud->points[cloudIdx[it]].valid == 0) {
      //invalid configuration, set both row and column of adjMatrix to 0
      adjMatrix.col(cloudIdx[it]) = Eigen::VectorXf::Zero(wConfigCount*lConfigCount*tConfigCount);
      adjMatrix.row(cloudIdx[it]) = Eigen::VectorXf::Zero(wConfigCount*lConfigCount*tConfigCount);
      continue;
    } else {
      PointXYZTP point;
      point.x = prmCloud->points[cloudIdx[it]].x;
      ...
      std::vector<float> pointDist;
      //ftree.radiusSearch(point,nSearchRadiusFactor*wheelWidth,pointIdx,pointDist,0);
      ftree.nearestKSearch(point,tConfigCount*9,pointIdx,pointDist);
//      ftree.nearestKSearch(point,63,pointIdx,pointDist);
      if(pointIdx.size()==0) {
        //no neighbor
        continue;
      }
      ...
      for (size_t i=0;i<pointIdx.size();++i) {
        if(prmCloud->points[pointIdx[i]].valid==0) {
          //neighbor config is invalid
          continue;
        }
        if(adjMatrix(cloudIdx[it],pointIdx[i])==0 || adjMatrix(pointIdx[i],cloudIdx[it])==0) {
          //neighbor already processed
            adjMatrix(cloudIdx[it],pointIdx[i]) = 0;
            adjMatrix(pointIdx[i],cloudIdx[it]) = 0;
          continue;
        }
        if(cloudIdx[it] == pointIdx[i]) {
          //neighbor is same as selected configuration
            adjMatrix(cloudIdx[it],pointIdx[i]) = 0;
            adjMatrix(pointIdx[i],cloudIdx[it]) = 0;
          continue;
        }
        if(prmCloud->points[cloudIdx[it]].x == prmCloud->points[pointIdx[i]].x && prmCloud->points[cloudIdx[it]].y == prmCloud->points[pointIdx[i]].y) {
          //neighbor has same x and y coordinates but different theta, cant be connected
            adjMatrix(cloudIdx[it],pointIdx[i]) = 0;
            adjMatrix(pointIdx[i],cloudIdx[it]) = 0;
          continue;
        }
        if(prmCloud->points[cloudIdx[it]].y == prmCloud->points[pointIdx[i]].y) {
          //neighbor has same y coordinates but different theta, cant be connected
            adjMatrix(cloudIdx[it],pointIdx[i]) = 0;
            adjMatrix(pointIdx[i],cloudIdx[it]) = 0;
          continue;
        }
        if(abs(prmCloud->points[cloudIdx[it]].theta - prmCloud->points[pointIdx[i]].theta)>20) {
          //difference between headings (theta) is high
            adjMatrix(cloudIdx[it],pointIdx[i]) = 0;
            adjMatrix(pointIdx[i],cloudIdx[it]) = 0;
          continue;
        }
        double xInc = (prmCloud->points[pointIdx[i]].x - point.x)/subConfigCount;
        double yInc = (prmCloud->points[pointIdx[i]].y - point.y)/subConfigCount;
        double zInc = (prmCloud->points[pointIdx[i]].z - point.z)/subConfigCount;
        double thetaInc = (prmCloud->points[pointIdx[i]].theta - point.theta)/subConfigCount;
        double phiInc = (prmCloud->points[pointIdx[i]].phi - point.phi)/subConfigCount;
        // check sub points between two neibouring nodes for validity
        for (size_t j=1;j<subConfigCount;++j) {
          PointXYZTP subPoint;
          subPoint.x = point.x+j*xInc;
     ...
     // cout<<"neighbor found :"<<nCount<<endl;
      degMatrix(cloudIdx[it],cloudIdx[it]) = nCount;
    }
  }
  lapMatrix = degMatrix - adjMatrix;
...
void RoadQuality::validateConfigs() {
  ...
  //cout<<"time per collision check"<<t2-t1<<endl;
 ...
 // cout <<"tyre position \t"<<tyrePos <<endl;
  for (size_t i=0;i<4;i++) {
   ...
   // cout <<"points under vehicle tyre \t"<<pointIdx.size() <<endl;
    if(pointIdx.size()<1) // not enough points udner tyre
    {
      //isValid = false;
      // break;
      continue;
    ...
    for (size_t k=0;k<pointIdx.size();k++)
    {
      ...
      if(distFromPlane<negObsTh || (distFromPlane > posObsTh && distFromPlane<vehHeight))
      {
        invalidPoint++;
      }else
      {
    ...
    {
      // cout <<"configuration invalid due to negative obstacles"<<endl;
      return false;
    }
  }
  //cout<<"valid so far...\n";
...
double cpu5  = clock() / CLOCKS_PER_SEC;
  ...
  // double cpu5  = clock() / CLOCKS_PER_SEC;
 // int counter =0;
  for (int i=obsIdx.size()-1;i>=0;i--) {
  //  cout <<"i=\t"<<i<<endl;
    //    Eigen::Vector3f confPoint(it->x,it->y,it->z);
    Eigen::Vector3f confPoint(cloudProjected->points[obsIdx[i]].x,cloudProjected->points[obsIdx[i]].y,cloudProjected->points[obsIdx[i]].z);
   ...
   // cout <<"distance is:"<<d1<<","<<d2<<","<<d3<<","<<d4<<endl;
    if (d1 < vehLength && d2 < vehWidth && d3 < vehLength &&  d4 < vehWidth) {
     // counter++;
      double distFromPlane = ((coefficients->values[0]*cloudXYZ->points[obsIdx[i]].x + coefficients->values[1]*cloudXYZ->points[obsIdx[i]].y + coefficients->values[2]*cloudXYZ->points[obsIdx[i]].z + coefficients->values[3])/(pCoeffMod));
      if (distFromPlane > vehicleGroundClearance && distFromPlane <vehHeight) {
          //std::cout<<"configuration invalid due to vehicle upper body with distance:\t"<<distFromPlane<<endl;
         // cout <<"point is :"<<cloudProjected->points[i].x<<","<<cloudProjected->points[i].y<<","<<cloudProjected->points[i].z<<endl;
          //cout <<"point is :"<<inCloud->points[i].x<<","<<inCloud->points[i].y<<","<<inCloud->points[i].z<<endl;
//  cout<<"index of obstacle\t"<<i<<endl;
  //if (i<obsIdx.size()-20) {
  //cout <<"about ordering reorder"<<endl;
    reorderPositiveObstacleIndex(i);
  //}
        return false;
      }
    } //else {
         //cout <<"point is :"<<cloudProjected->points[i].x<<","<<cloudProjected->points[i].y<<","<<cloudProjected->points[i].z<<endl;
        //cout <<"distance is:"<<d1<<","<<d2<<","<<d3<<","<<d4<<endl;
         // cout<<"vehicle box is:"<<vehicleBox<<endl;
    //}
  }
...
if (argc < 11)
  ...
  string dataFolder = "/home/mudassir/phd_ws/data/traversability/pointclouds/kitti/2011_09_26_drive_0086/";
  string prmFolder = "/home/mudassir/phd_ws/data/traversability/prm/kitti/drive0086/v2/";
  string adjFolder = "/home/mudassir/phd_ws/data/traversability/adjacency/kitti/drive0086/v2/";
    ...
    while ((ent = readdir (dir)) != NULL) {
      ...
      if(fName.find("road_")!=std::string::npos)
      {
          ...
          while ((entPRM = readdir (dirPRM)) != NULL) {
               string fNamePRM = entPRM->d_name;
              if(fNamePRM.find("road_")!=std::string::npos) {
             string fProcessed = fNamePRM.substr(0,fNamePRM.find("_prm_view.pcd"));
           string f2Process = fName.substr(0,fName.find(".pcd"));
        ...
        obj.setTreeCloud();
        obj.setOcTreeCloud();
        obj.getPositiveObstacleIndex();
        cout <<"check configurations validity \n";
        obj.validateConfigs();
        obj.setfTreeCloud();
        cout <<"generating PRM graph \n";
        obj.generatePRM();
        //obj.getStartConfigurations("deterministic");
        //obj.getGoalConfigurations("deterministic");
        ...
        {
            for (size_t i=0;i<obj.lConfigCount*obj.wConfigCount*obj.tConfigCount;i++) {
              for (size_t j=0;j<obj.lConfigCount*obj.wConfigCount*obj.tConfigCount;j++) {
                if (obj.adjMatrix(i,j)==1) {
                    saveAdjFile <<i<<" "<<j<<" "<<1<< "\n";
                }
              }
            }
          saveAdjFile.close();
            ...
            cout<<"Error: can not find directory"<<endl;
        }

        std::stringstream saveDeg;
        saveDeg << adjFolder.c_str() << fName.substr(0,fName.find(".pcd")) <<"_deg";
...
//        std::stringstream saveStart;
//        saveStart << adjFolder.c_str() << fName.substr(0,fName.find(".pcd")) <<"_start";
//        std::ofstream saveStartFile(saveStart.str().c_str());
//        if (saveStartFile.is_open())
//        {
//          saveStartFile << obj.startVector << "\n";
//          saveStartFile.close();
//        }

//        std::stringstream saveGoal;
//        saveGoal << adjFolder.c_str() << fName.substr(0,fName.find(".pcd")) <<"_goal";
//        std::ofstream saveGoalFile(saveGoal.str().c_str());
//        if (saveGoalFile.is_open())
//        {
//          saveGoalFile << obj.goalVector << "\n";
//          saveGoalFile.close();
//        }

        std::stringstream savePRMFile;
        std::stringstream savePRMViewFile;
        savePRMViewFile << prmFolder.c_str() << fName.substr(0,fName.find(".pcd")) << "_prm_view.pcd";
        savePRMFile << prmFolder.c_str() << fName.substr(0,fName.find(".pcd")) << "_prm.pcd";
